...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $results = $database->execute_query("SELECT * FROM contacts WHERE nome LIKE :search OR telefone like :search ORDER BY id DESC", $params);
} else {
    $results = $database->execute_query("SELECT * FROM contacts ORDER BY id DESC");
}
...
<div class="row">
    <div class="col">
        ...
        <?php if($total_contacts == 0): ?>
            <!-- no results -->
            <p class="text-center opacity-75 mt-3">Não foram encontrados contacto registados.</p>
        <?php else: ?>
