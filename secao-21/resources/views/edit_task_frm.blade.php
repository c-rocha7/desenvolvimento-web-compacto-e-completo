            ...
            <div class="col">
                ...
                <form action="{{ route('edit_task_submit') }}" method="post">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="text_task_description" id="text_tesk_description" class="form-control" rows="5" required>{{ old('text_task_description', $task->task_description) }}</textarea>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="text_task_status" id="text_task_status" class="form-select w-25" required>
                            <option value="new" {{ old('text_task_status', $task->status) == 'new' ? 'selected' : '' }}>
                                Nova
                            </option>
                            <option value="in_progress"
                                {{ old('text_task_status', $task->status) == 'in_progress' ? 'selected' : '' }}>Em progresso
                            </option>
                            <option value="cancelled"
                                {{ old('text_task_status', $task->status) == 'cancelled' ? 'selected' : '' }}>Cancelada</option>
