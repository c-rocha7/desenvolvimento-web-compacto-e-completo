...
class Agents extends BaseModel
{
    ...
    {
        // check if exists another agent's client with same name
        $params = [
            ':id'       => $id,
            ':name'     => $name,
            ':id_agent' => $_SESSION['user']->id,
        ...
        $this->db_connect();
        $results = $this->query("SELECT id FROM persons WHERE id <> :id AND id_agent = :id_agent AND AES_ENCRYPT(:name, '".MYSQL_AES_KEY."') = name", $params);
    ...
    // =======================================================
    public function set_agent_password($id, $new_password)
    {
        // updates the current user password
        $params = [
            ':passwrd' => password_hash($new_password, PASSWORD_DEFAULT),
